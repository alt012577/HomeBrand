  ...
  <div>
    <div id="barnds" style="height: 98%;">
		...
		<!-- 	<span>首页</span>
			...
			<span @click="backserversMap" v-if="cityname!=''">全国服务商</span>
			<span class="arrow" v-if="cityname!=''">&gt;</span>
			<span @click="backserversCityMap" v-if="cityname!=''">{{cityname}}</span>
      <span @click="backserversMap" v-if="cityname==''">全国服务商</span>
			<span class="arrow">&gt;</span>
			<span>{{areaname}}</span>
		</div>
...
<script>
  import $ from 'jquery'
  export default{
    props: ["regionInfoID","serveregionInfoName","provinceNameH"],
    data() {
      return{
        marker: [],
        map: null,
    ...
    mounted() {
	  ...
	  this.areaname = this.serveregionInfoName
    },
    methods: {
		backserversMap() {
		...
		backserversCityMap() {
			this.$emit("backserversCityMap")
		},
      initCityMap() {
        let _this = this
        $.ajax({
          url: baseURL + "WholeDecoration/GetServiceProviderPageList_WDBoss",
          data: {
              WholeDecorationID: _this.WholeDecorationID,
              memberGUID: _this.memberGUID,
              regionInfoID: _this.regionInfoID
          },
          success: res => {
            if( res.code == 1 ){
              if( res.data == null ){
      ...
      areaMap(list) {
          ...
          if( item.orderSL >= 0 ){
            _this.marker[index].setLabel({
              content: "<div><p>"+item.organizationName+" ("+item.organizationAlias+")"+"</p><div><span>下单数量</span><span>"+item.orderSL+"</span></div></div>", //设置文本标注内容
