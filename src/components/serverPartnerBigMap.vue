  ...
  export default{
    ...
    methods: {
      ...
      initBrandList() {
        let _this = this
        $.ajax({
          url: baseURL + "WholeDecoration/GetNationwideSPList_WDBoss",
          data: {
      ...
      serverPartnerBigMapbar(list) {
        let canvas = document.getElementById('specialbar')
        var myChart = echarts.init(canvas);

        let serverPartnerBigMapbar = function(list) {
          let obj = {
            datax: [],
            data1: [],
            data2: []
        ...
        let serverPartnerBigMapbars = serverPartnerBigMapbar(list)
        ...
        let option = {
            ...
            xAxis: [
                {
                    type: 'category',
                    data: serverPartnerBigMapbars.datax,
                    axisPointer: {
                        type: 'shadow'
                    },
                    axisLine: {
            ...
            series: [
                {
                    ...
                    data: serverPartnerBigMapbars.data1
                },
                {
                    name: '下单数量',
                    type: 'bar',
                    ...
                    data: serverPartnerBigMapbars.data2
                },
            ]
        };
		...
		myChart.on('click', function (param) {
		    let province = {}
		    province.provinceID = param.data.provinceID
		    province.provinceName = _this.pinyin.getFullChars(param.name).toLowerCase()
		    province.provinceNameH = param.name
		    _this.$emit("changeProvinceID",province);
		});
      },
      map(list) {
        let _this = this
        ...
        $.getJSON(uploadedDataURL, function(geoJson) {
            ...
            list.forEach((item,index)=>{
              ...
              obj.value1 = item.providerSL
              obj.value2 = item.orderSL
              obj.provinceID = item.provinceID
              data.push(obj)
            })

            _this.serverPartnerBigMapbar(data)
